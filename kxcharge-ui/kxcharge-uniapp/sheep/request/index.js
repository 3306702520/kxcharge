...
http.interceptors.request.use(
	(config) => {
		...
		config.header['tag'] = 'PC-202401161832';
		if (config.url.indexOf('tenant/get-by-website') < 0) {
			config.header['Tenant-Id'] = handleTenantId()
		}
		return config;
	},
	(error) => {
...
http.interceptors.response.use(
	(response) => {
		...
		if (response.data.code !== 0) {
			...
			// 错误提示
			if (response.config.custom.showError && response.data.msg.indexOf('租户标识未传递') < 0) {
				uni.showToast({
