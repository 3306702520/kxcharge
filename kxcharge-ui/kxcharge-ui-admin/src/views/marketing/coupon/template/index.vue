  ...
  <ContentWrap>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          v-hasPermi="['operations:coupon-template:create']"
          plain
          type="primary"
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      >
        <template #default="scope">
          <el-button
            v-hasPermi="['operations:coupon-template:update']"
            link
            type="primary"
          ...
          <el-button
            v-hasPermi="['operations:coupon-template:delete']"
            link
            type="danger"
...
import {
  ...
  validityTypeFormat
} from '@/views/marketing/coupon/formatter'

defineOptions({ name: 'MarketingCouponTemplate' })
...
const handleStatusChange = async (row: any) => {
  ...
  try {
    await message.confirm('确认要"' + text + '""' + row.name + '"优惠券吗?')
...
const handleDelete = async (id: number) => {
  try {
    // 删除的二次确认
    await message.confirm('是否确认删除优惠券编号为"' + id + '"的数据项?')
    // 发起删除
