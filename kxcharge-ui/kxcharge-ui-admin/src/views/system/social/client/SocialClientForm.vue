  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    ...
    >
      <el-form-item label="应用名" prop="name">
        <el-input v-model="formData.name" v-no-space placeholder="请输入应用名" />
      </el-form-item>
      ...
      <el-form-item label="客户端编号" prop="clientId">
        <el-input v-model="formData.clientId" v-no-space placeholder="请输入客户端编号,对应各平台的appKey" />
      </el-form-item>
      ...
      <el-form-item label="agentId" prop="agentId" v-if="formData!.socialType === 30 || formData!.socialType === 40">
...
const contentValidator = (rule: any, _value: any, callback: any) => {
  if (_value !== 0 && (!_value || !_value.trim())) {
...
const formRules = reactive({
  name: [{ required: true, message: '内容不能为空', validator: contentValidator, trigger: 'blur' }],
  socialType: [{ required: true, message: '社交平台不能为空', trigger: 'blur' }],
  userType: [{ required: true, message: '用户类型不能为空', trigger: 'blur' }],
  clientId: [{ required: true, mssage: '内容不能为空', validator: contentValidator, trigger: 'blur' }],
  clientSecret: [{ required: true, message: '内容不能为空', validator: contentValidator, trigger: 'blur' }],
