<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle">
    ...
    >
      ...
      <el-form-item v-if="formData.type === 2" label="跳转参数" prop="pathArgument">
        <template #label>
          <Tooltip
            message="clientId:客户端编号,toUrl:跳转到的链接(不加协议),dashboard:仪表板Id"
            title="跳转参数"
          />
        </template>
        <el-input v-model="formData.pathArgument" clearable placeholder="例如说：clientId=fudaoTb,toUrl=xxx(不加协议)" />
      </el-form-item>
