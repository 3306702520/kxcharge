  ...
  <Dialog v-model="dialogVisible" :title="dialogTitle" width="800">
    ...
    >
      <el-form-item label="公告标题" prop="title">
        <el-input v-model="formData.title" v-no-space placeholder="请输入公告标题" />
      </el-form-item>
      <el-form-item label="公告内容" prop="content">
        <Editor v-model="formData.content" height="150px" ref="editor" />
      </el-form-item>
...
const contentValidator = (rule: any, _value: any, callback: any) => {
  console.log(`output->_value`,_value)
  const str = _value.replace(/<[^<>]+>/g,"").replace(/&nbsp;/gi,"")
  const hasImage = /<img\s+[^>]*src="([^"]*)"/gi.test(_value);
  if (_value !== 0 && (!_value || !_value.trim() || !str || !str.trim()) && !hasImage) {
...
const formRules = reactive({
  title: [{ required: true, validator: contentValidator, trigger: 'blur' }],
