<template>
  ...
  >
    ...
    <el-table-column align="center" label="所需积分" min-width="168">
      <template #default="{ row }">
        <el-input-number v-model="row.integralPrice" :min="0" :precision="0" :step="1" class="w-100%" />
      </template>
    </el-table-column>
  ...
  >
    ...
    <el-table-column align="center" label="所需积分" min-width="80">
      <template #default="{ row }">
        {{ row.integralPrice }}
      </template>
    </el-table-column>
...
const batchAdd = () => {
  if (!formData.value!.skus?.length) {
    message.info('属性列表为空，不能批量添加! 添加属性值后再试!')
    return
  }
...
watch(
  ...
  (data) => {
    if (!data) return
    formData.value = data
    console.log(`output->formData.valu1111`,formData.value)
  },
  {
    deep: true,
    immediate: true
...
watch(
  ...
  (propertyList: PropertyAndValues[]) => {
    console.log(`output->propertyList`,propertyList)
    // 如果不是多规格则结束
