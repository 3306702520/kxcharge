<template>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="通联支付配置" align="center">
        <el-table-column :label="PayChannelEnum.TONGLIAN.name" align="center">
          <template #default="scope">
            <el-button
              type="success"
              circle
              v-if="isChannelExists(scope.row.channelCodes, PayChannelEnum.TONGLIAN.code)"
              @click="openChannelForm(scope.row, PayChannelEnum.TONGLIAN.code)"
            >
              <Icon icon="ep:check" />
            </el-button>
            <el-button
  ...
  <TonglianChannelForm ref="tonglianFormRef" @success="getList" />
</template>
...
import TonglianChannelForm from './components/channel/TonglianChannelForm.vue'
...
const openChannelForm = async (row, payCode) => {
  ...
  if (payCode.indexOf('tonglian') === 0) {
    tonglianFormRef.value.open(row.id, payCode)
  }
}
