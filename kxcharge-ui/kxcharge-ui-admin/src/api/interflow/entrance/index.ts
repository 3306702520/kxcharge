...
export const getEntrancePage = async (params) => {
  return await request.get({ url: `/interflow/entrance/page`, params })
}

// 查询入口方案详情
export const getEntrance = async (id: number) => {
  return await request.get({ url: `/interflow/entrance/get?id=` + id })
}

// 新增入口方案
export const createEntrance = async (data: EntranceVO) => {
  return await request.post({ url: `/interflow/entrance/create`, data })
}

// 修改入口方案
export const updateEntrance = async (data: EntranceVO) => {
  return await request.put({ url: `/interflow/entrance/update`, data })
}

// 删除入口方案
export const deleteEntrance = async (id: number) => {
  return await request.delete({ url: `/interflow/entrance/delete?id=` + id })
}

// 导出入口方案 Excel
export const exportEntrance = async (params) => {
  return await request.download({ url: `/interflow/entrance/export-excel`, params })
}
