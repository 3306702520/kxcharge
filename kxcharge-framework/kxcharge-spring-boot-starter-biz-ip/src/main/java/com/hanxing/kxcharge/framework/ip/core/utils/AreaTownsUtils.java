 ...
 */
@Slf4j
public class AreaTownsUtils {

    /**
     * 初始化 SEARCHER
     */
    @SuppressWarnings("InstantiationOfUtilityClass")
    private final static AreaTownsUtils INSTANCE = new AreaTownsUtils();
    ...
    private AreaTownsUtils() {
        long now = System.currentTimeMillis();
        areas = new HashMap<>();
        areas.put(Area.ID_GLOBAL, new Area(Area.ID_GLOBAL, "全球", 0,
                null, new ArrayList<>()));
        // 从 csv 中加载数据
        List<CsvRow> rows = CsvUtil.getReader().read(ResourceUtil.getUtf8Reader("areaTowns.csv")).getRows();
        ...
        for (CsvRow row : rows) {
            Area area = areas.get(String.valueOf(row.get(0))); // 自己
            Area parent = areas.get(String.valueOf(row.get(3))); // 父
    ...
    public static String getParentIdByType(String id, @NonNull AreaTypeEnum type) {
        for (int i = 0; i < Byte.MAX_VALUE; i++) {
            Area area = AreaTownsUtils.getArea(id);
            if (area == null) {
                return null;
            }
            // 情况一：匹配到，返回它
